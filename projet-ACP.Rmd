...
- \fancyhead{}
- \fancyfoot{}
- \fancyfoot[C]{\hyperlink{page.1}{TOC}}
- \fancyfoot[R]{\thepage}
...
var_plot <- ggplot(dbvar,aes(x= Dim.1,y= Dim.2,label=names, color = type))+
  ...
  geom_text_repel(size=3,show.legend = FALSE, max.overlaps = 5)+
  ...
  ggtitle("Figure I - Espace des variables de l'ACP \npour les formations universitaires françaises", ) +
  xlab(paste("Axe 1 :",round(resultat_ACP$eig[1,2],0),"%"))+
  ylab(paste("Axe 2 :",round(resultat_ACP$eig[2,2],0),"%")) +
  geom_circle( aes(x0 = 0, y0 = 0, r = 1), inherit.aes = F, size = 0.25) + 
  scale_x_continuous(limits = c(-1, 1)) +
  scale_y_continuous(limits = c(-1, 1)) +
  coord_fixed() +
  geom_segment(aes(x = 0, y = 0, xend = Dim.1, yend = Dim.2, linetype = type), arrow = arrow(), size = 0.5)
...
# 1. Présentation des résultats de l'ACP à l'échelle nationale. 
...
A vérifier : la variable "niveau" représentée notamment dans le graphique des individus affiche des valeurs anormales : M6, L5,etc. 
De plus, certains établissements n'ont pas de libellé, ce qui rend difficilement lisibles leur représentation graphique. 

\newpage

# 2. Etablissements

```{r demo, results='asis', fig.width=12, fig.height=8, fig.align= "center"}
for(academie in c("Académie de Paris", "Académie d'Aix-Marseille", "Académie de Besançon", "Académie de Bordeaux",
  ...
  print(graphique(academie))
}
```

# 3. Ebauche de commentaire
