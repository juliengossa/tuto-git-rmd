...
ACP <- function(academie) {
  db <- sise %>%
    ...
    group_by(ETABLI,discipline) %>%
    summarize(academie = first(ACANOM),
...
pour_PCA<-db %>% 
  ...
  select(-effectif,-ETABLI,-academie, -discipline) %>% 
  relocate(c("hommes","femmes","doctorats","masters","licences", "Libellé", "Sigle"),.after= prof_inter)

resultat_ACP<-PCA(pour_PCA[,2:18], quanti.sup= 13:17, row.w=pour_PCA$poids, graph = FALSE)
...
var_plot <- ggplot(dbvar,aes(x= Dim.1,y= Dim.2,label=names))+
...
ind_plot <- ggplot(data=dbind,aes(x=Dim.1,y=Dim.2))+
  ggtitle(paste0("Figure II - Espace des formations universitaires de l'", academie))+
  geom_point()+
  geom_text_repel(aes(label=paste(Sigle, discipline),size=3))+
  ...
  labs(caption = "L'analyse en composantes principales est menée sur une aggrégation par établissement et discipline.")+
  xlab(paste("Axe 1 :",round(resultat_ACP$eig[1,2],0),"%"))+
  ylab(paste("Axe 2 :",round(resultat_ACP$eig[2,2],0),"%"))
